...
const CustomersHistory = ({ onClose }) => {
    ...
    useEffect(() => {
        fetch(`${backendURL}/customers/history`)
            .then((res) => res.json())
            .then((data) => {
                setHistory(data);
    ...
    return (
        <div className="p-6 bg-white overflow-y-auto w-full h-full">
            ...
            <div className="mb-2">
                <label className="mr-2 text-gray-600 font-medium">Filter by Date:</label>
            ...
            <div className="space-y-4">
                {filteredHistory.length === 0 ? (
                    <p className="text-center text-gray-500">No customer history available</p>
                ) : (
                    filteredHistory.map((record, index) => {
                        ...
                        return (
                            <div key={index} className="p-4 border rounded-lg shadow-md bg-gray-50">
                                <h2 className="text-[16px]">Customer Name: <span className="text-blue-500 font-medium">{record.customerName}</span></h2>
                                <div className="flex flex-row justify-between items-center text-[16px]">
                                    ...
                                    <h3 className="text-gray-500">
                                        Modified At: {record.modifiedAt ? new Date(record.modifiedAt).toLocaleString() : "N/A"}
                                    </h3>
